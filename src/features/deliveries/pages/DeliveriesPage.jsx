...
export function Deliveries() {
  ...
  const getAll = () => {
    ...
    let all = [];
    try {
      const raw = Cookies.get("slashit_attendant_deliveries");
    ...
    legacy.forEach((d) => {
      if (!ids.has(d.id)) all.push(d);
    });
    return hubId
      ? all.filter((d) => !d.hubId || String(d.hubId) === String(hubId))
      : all;
  };
  ...
  const save = (updated) => {
    ...
    try {
      ...
      const merged = [
        ...updated,
        ...existing.filter((d) => !updatedIds.has(d.id)),
      ];
      Cookies.set("slashit_attendant_deliveries", JSON.stringify(merged));
    } catch (e) {}
  ...
  const historyDeliveries = allDeliveries
    .filter((d) => d.status === "completed")
    .sort(
      (a, b) =>
        new Date(b.completedAt || b.verifiedAt || 0) -
        new Date(a.completedAt || a.verifiedAt || 0),
    );
  ...
  const markArrived = async (id) => {
    ...
    save(
      allDeliveries.map((x) =>
        x.id === id
          ? { ...x, status: "arrived", arrivedAt: new Date().toISOString() }
          : x,
      ),
    );
    ...
    pushBrowserNotif(
      "📦 Delivery Arrived",
      allDeliveries.find((d) => d.id === id)?.productName +
        " has arrived at " +
        att?.hubName,
    );
  };
  ...
  const verify = async (id) => {
    ...
    save(
      allDeliveries.map((x) =>
        x.id === id ? { ...x, status: "verified", verifiedAt: now } : x,
      ),
    );
    if (delivery?.slashId) {
      try {
        ...
        const updated = userSlashes.map((s) =>
          String(s.id) === String(delivery.slashId)
            ? { ...s, status: "ready_for_pickup", verifiedAt: now }
            : s,
        );
        Cookies.set("slashit_user_slashes", JSON.stringify(updated));
      } catch (e) {}
    }
    toast.success(
      'Delivery verified ✅ — tap "Notify Slashers" to alert members',
    );
  };

  const sendPickupNotification = async (delivery) => {
    if (!pickupFrom || !pickupTo) {
      toast.error("Set pickup time window");
      return;
    }
    setSending(true);
    await delay(500);
    ...
    const col = cols.find(
      (c) => String(c.slashId) === String(delivery.slashId),
    );
    if (col) {
      ...
      pending.forEach((member) => {
        ...
        msgStore.push(member.userId, {
          id: `msg-pickup-${Date.now()}-${member.userId}`,
          type: "pickup_ready",
          slashId: col.slashId,
        ...
        notifStore.push(member.userId, {
          id: `notif-pickup-${Date.now()}-${member.userId}`,
          type: "pickup_ready",
          title: "📦 Ready for Pickup!",
          body: `Your ${col.productName} is ready at ${att?.hubName || col.hubName}. Collect ${pickupFrom}–${pickupTo}.`,
          isRead: false,
        ...
        pushBrowserNotif(
          "📦 Ready for Pickup!",
          `Your ${col.productName} is ready at ${att?.hubName || col.hubName}. Collect ${pickupFrom}–${pickupTo}.`,
        );
      });
      adminLog.push({
        id: `log-${Date.now()}`,
        ...
        pickupFrom,
        pickupTo,
        membersNotified: pending.length,
        totalMembers: col.members.length,
        deliveryVerifiedAt: delivery.verifiedAt,
        sentAt: new Date().toISOString(),
      });
      toast.success(
        `📨 ${pending.length} member${pending.length !== 1 ? "s" : ""} notified!`,
      );
    } else {
  ...
  const reportIssue = async (id) => {
    ...
    try {
      ...
      Cookies.set(
        ...
        JSON.stringify([
          {
            id: "att-issue-" + Date.now(),
            source: "attendant",
            attendantName: att?.name || "Attendant",
            hubName: att?.hubName || "",
            slashId: delivery?.slashId,
            slashName: delivery?.productName,
            reason: reportNote.trim(),
            status: "open",
            priority: "high",
            createdAt: new Date().toISOString(),
            emoji: delivery?.productEmoji || "📦",
          },
          ...existing,
        ]),
      );
  ...
  const renderCard = (d, isHistory = false) => {
    ...
    const payments = (() => {
      try {
        return JSON.parse(
          Cookies.get("slashit_admin_supplier_payments") || "[]",
        );
      } catch (e) {
        return [];
      }
    ...
    const isPaid = payments.some(
      (p) => String(p.slashId) === String(d.slashId),
    );
    const slashStatus = (() => {
      try {
        const s = JSON.parse(Cookies.get("slashit_user_slashes") || "[]");
        return s.find((sl) => String(sl.id) === String(d.slashId))?.status;
      } catch (e) {
        return null;
      }
    })();
        ...
        <div style={{ padding: 16 }}>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>
                {d.quantity} · {d.supplierName}
              </div>
              <div style={{ fontSize: 12, color: "#64748b" }}>
                📞 Leader: {d.leaderName} ({d.leaderPhone})
              </div>
            </div>
          ...
          <div
            style={{
              ...
              fontSize: 12,
              color: "#64748b",
              marginBottom: isHistory ? 0 : 12,
            }}
          >
            <div style={{ fontWeight: 600, color: "#1e293b", marginBottom: 4 }}>
              Expected: {d.expectedItems}
            </div>
            <div>
              Value:{" "}
              <strong style={{ color: "#1d4ed8" }}>{fmt(d.totalValue)}</strong>
            </div>
            {d.scheduledAt && (
              <div style={{ marginTop: 2 }}>
                Scheduled: {fmtTime(d.scheduledAt)}
              </div>
            )}
            {d.arrivedAt && (
              <div style={{ marginTop: 2, color: "#1d4ed8" }}>
                Arrived: {fmtTime(d.arrivedAt)}
              </div>
            )}
            {d.verifiedAt && (
              <div style={{ marginTop: 2, color: "#16a34a", fontWeight: 700 }}>
                ✓ Verified: {fmtTime(d.verifiedAt)}
              </div>
            )}
            {d.completedAt && (
              <div style={{ marginTop: 2, color: "#15803d", fontWeight: 700 }}>
                🎉 Completed:{" "}
            ...
            {d.memberCount && (
              <div style={{ marginTop: 2 }}>
                Members:{" "}
                <strong>
                  {d.collectedCount || 0}/{d.memberCount}
                </strong>{" "}
                collected
              </div>
            )}
            ...
            <>
              {notifying === d.id ? (
                ...
                >
                  ...
                  >
                    {[
                      ["From", pickupFrom, setPickupFrom],
                      ["To", pickupTo, setPickupTo],
                  ...
                  >
                    Preview: "Your {d.productName} is ready! Come collect
                    between {pickupFrom}–{pickupTo} today."
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <Btn
                      ...
                      full
                      loading={sending}
                      onClick={() => sendPickupNotification(d)}
                    >
                      📨 Send to Members
                    </Btn>
                    <Btn
                ...
                <div style={{ marginTop: 12 }}>
                  <textarea
                    ...
                    rows={2}
                    placeholder="Describe the issue (e.g. short delivery, damaged goods...)"
                ...
                >
                  {d.status === "arriving" && paymentSent && (
                    <Btn
                      size="sm"
                      variant="primary"
                      full
                      onClick={() => markArrived(d.id)}
                    >
                      Mark as Arrived
                    </Btn>
                  )}
                  {d.status === "arriving" && !paymentSent && (
                    ...
                    >
                      ⏳ Waiting for admin to pay supplier
                    </div>
                  )}
                  {d.status === "arrived" && (
                    <Btn
  ...
  return (
    <div style={{ background: "#f0f4ff", minHeight: "100vh" }}>
      ...
      >
        ...
        >
          Incoming orders for {att?.hubName}
        </div>
        {/* Tabs */}
          ...
          (activeDeliveries.length === 0 ? (
            <div style={{ textAlign: "center", padding: 48, color: "#94a3b8" }}>
              ...
              <div style={{ fontSize: 12, marginTop: 6 }}>
                New deliveries appear here automatically when slashes fill up
              </div>
            </div>
          ) : (
        ...
        {tab === "history" &&
          (historyDeliveries.length === 0 ? (
            <div style={{ textAlign: "center", padding: 48, color: "#94a3b8" }}>
              ...
              <div style={{ fontSize: 12, marginTop: 6 }}>
                Completed deliveries will appear here once all members collect
              </div>
            </div>
          ) : (
