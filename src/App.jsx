...
function BottomNav() {
  ...
  useEffect(() => {
    const count = () => {
      ...
      setUnread(
        msgs.filter((m) => m.from === "user" && !m.attendantRead).length,
      );
  ...
  return (
    <>
      <div className="att-spacer" style={{ height: 72 }} />
      <nav
        className="att-bottom-nav"
        style={{
          position: "fixed",
          bottom: 0,
          left: "50%",
...
function Login() {
  ...
  const login = async () => {
    ...
    try {
      ...
      toast.success(
        "Welcome, " +
          (currentAttendant.name || "Attendant").split(" ")[0] +
          "! 👋",
      );
      nav("/dashboard");
    } catch (err) {
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 14, color: "#64748b" }}>
            Sign in to your hub management account
          </div>
        </div>
        <div
          style={{
...
function Protected({ children }) {
  ...
  return (
    <>
      ...
      <AttendantSidebar />
      {/* Main content — CSS adds margin-left on desktop. Make it scrollable on mobile. */}
      <div
        className="att-main"
        style={{
          overflowY: "auto",
